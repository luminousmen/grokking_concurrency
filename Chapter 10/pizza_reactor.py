...
class EventLoop:
    def __init__(self):
        ...
        # I/O multiplexing: select and selectors. select is more low level and
        # expose you to the specific select-family syscalls, selectors is a
        # higher level module that choose the best implementation on your
        # system, roughly epoll|kqueue|devpoll > poll > select
        self.event_notifier = selectors.DefaultSelector()

    def register_event(self, socket: socket, event: int, callback: Callable):
        try:
    ...
    def unregister_event(self, socket: socket):
        self.event_notifier.unregister(socket)
    ...
    def run_forever(self) -> None:
        while True:
            ...
            for key, mask in events:
                # key.fileobj is a socket object, and the mask is the event
                # mask of the operations that are ready
...
class Server:
    ...
    def _on_read(self, conn: socket):
        data = conn.recv(BUFFER_SIZE)
        if data:
            message = data.decode()
            self.event_loop.register_event(
                conn, selectors.EVENT_WRITE, (self._on_write, message))
        else:
            self.event_loop.event_notifier.unregister(conn)
            conn.close()
            print(f"Connection with {conn.getpeername()} has been closed")

    def _on_write(self, conn: socket, message: str) -> None:
